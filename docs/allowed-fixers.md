...
Only the existing `.gitignore` housekeeping class can satisfy `automatic_merge_eligible`. `npm-security-update`, `python-security-pin`, and `container-digest-pin` may be proposed in `pr-only` or `automerge-low-risk`, but their pull requests remain draft and require human review. `automerge-low-risk` does not expand their authority.
